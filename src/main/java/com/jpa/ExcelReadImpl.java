...
public class ExcelReadImpl implements ExcelDAO {
	...
	public void createEventCause(HSSFWorkbook wb) {
		...
		try {
			...
			for (int i = 0; i < col.size(); i++) {
				eventCause = new EventCause(col.get(i).intValue(), col1.get(i));
				em.merge(eventCause);
			}
		} catch (InvalidFormatException e) {
			e.getMessage();
	...
	public void createFault(HSSFWorkbook wb) {
		...
		try {
			 ...
			 */
			for (int i = 0; i < col.size(); i++) {
				fault = new Fault(col.get(i).toString(),
						col1.get(i).intValue(),
						dao.getByFailure(col2.get(i).intValue()),
						dao.getByTac(col3.get(i).intValue()),
						dao.getByMCC(col4.get(i).intValue()),
						col5.get(i).intValue(),
						dao.getByCellId(col6.get(i).intValue()),
						col7.get(i).intValue(),
						dao.getByEventCause(col8.get(i).intValue()),
						col9.get(i),
